  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        size === "sm" && "h-7 rounded-[min(var(--radius-md),10px)] py-0",
        size === "default" && "h-8 py-2",
        size === "lg" && "h-11 min-h-11 rounded-xl px-4 py-0 text-xs font-bold leading-none",
        className
      )}
      {...props}
    >
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Positioner
        ...
        className={cn("isolate z-[130]", positionerClassName)}
      >
        <SelectPrimitive.Popup
          data-slot="select-content"
          data-align-trigger={alignItemWithTrigger}
          className={cn("relative isolate z-50 max-h-(--available-height) w-(--anchor-width) min-w-36 origin-(--transform-origin) overflow-x-hidden overflow-y-auto rounded-lg bg-popover text-popover-foreground shadow-md ring-1 ring-foreground/10 duration-100 dark:bg-[#252849] dark:text-slate-100 dark:ring-white/10 data-[align-trigger=true]:animate-none data-[side=bottom]:slide-in-from-top-2 data-[side=inline-end]:slide-in-from-left-2 data-[side=inline-start]:slide-in-from-right-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-open:animate-in data-open:fade-in-0 data-open:zoom-in-95 data-closed:animate-out data-closed:fade-out-0 data-closed:zoom-out-95", className )}
          {...props}
        >
