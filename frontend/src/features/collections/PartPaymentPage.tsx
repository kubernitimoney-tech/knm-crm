...
export const PartPaymentPage = () => {
  useTitle('Part Payment');
  ...
  const { rows: partPaymentRows, isLoading, refetch } = usePipelineRows('part-payment');
  ...
  const filteredAndSorted = useMemo(() => {
    const result = partPaymentRows.filter(lead => {
      const matchesSearch =
    ...
    return result;
  }, [partPaymentRows, searchQuery, dateFilter, dateRange, sortConfig]);
  ...
  const handleExport = () => {
    ...
    exportRowsToExcel(headers, data, `part_payment_records_${format(new Date(), 'yyyy-MM-dd')}`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Part Payment</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">Accounts with pending partial repayments</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="overflow-x-auto overflow-y-hidden">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading part payments…" />
              ) : paginated.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 text-center">
                    <p className="text-slate-400 font-bold text-sm">No partial payment records found</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
