...
export const ClosedAccountsPage = () => {
  ...
  const { rows: closedRows, isLoading, refetch } = usePipelineRows('closed');
  ...
  const filteredAndSorted = useMemo(() => {
    const result = closedRows.filter(lead => {
      const matchesSearch =
    ...
    return result;
  }, [closedRows, searchQuery, dateFilter, dateRange, sortConfig]);
  ...
  const handleExport = () => {
    ...
    exportRowsToExcel(headers, data, `closed_accounts_${format(new Date(), 'yyyy-MM-dd')}`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Closed Accounts</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">History of successfully completed loan cycles</p>
        </div>
      ...
      <div className="space-y-3">
        <div className={dataTableFilterToolbarClass}>
          <DataTableSearchInput
            placeholder={searchPlaceholder('Name', 'Mobile', 'or Loan ID')}
            value={searchQuery}
            onChange={(value) => {
          ...
          <Table>
            <TableHeader className={dataTableHeaderClass}>
              <TableRow className={dataTableHeaderRowClass}>
                ...
                <TableHead className={cn(dataTableHeadClass, 'text-right')}>Payment Amt.</TableHead>
                ...
                <TableHead className={cn(dataTableHeadClass, 'text-right')}>Discount Amt.</TableHead>
                <TableHead className={cn(dataTableHeadClass, 'text-right')}>Settled Amt.</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading closed accounts…" />
              ) : paginated.length > 0 ? (
                paginated.map((lead, idx) => (
                  <TableRow key={lead.id} className={dataTableBodyRowClass}>
                    ...
                    <TableCell className={dataTableCellClass}>{lead.pancard}</TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 text-center">
                    <p className="text-slate-400 font-bold text-sm">No closed accounts found</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
