...
export const SettlementPage = () => {
  ...
  const { rows: settlementRows, isLoading, refetch } = usePipelineRows('settlement');
  ...
  const filteredAndSorted = useMemo(() => {
    const result = settlementRows.filter(lead => {
      const matchesSearch =
        ...
        lead.refNo?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    return result;
  }, [settlementRows, searchQuery, dateFilter, dateRange, sortConfig]);
  ...
  const handleExport = () => {
    ...
    exportRowsToExcel(headers, data, `settlement_accounts_${format(new Date(), 'yyyy-MM-dd')}`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Settlement</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">
            Loans closed under settlement with write-off
          </p>
        </div>
        <div className="flex items-center gap-2">
          <ExportButton permission="collection.export" onClick={handleExport} />
        </div>
      </div>
          ...
          <Table>
            <TableHeader className={dataTableHeaderClass}>
              <TableRow className={dataTableHeaderRowClass}>
                ...
                <TableHead className={dataTableHeadClass}>Mobile</TableHead>
                <TableHead className={dataTableHeadClass}>PAN No.</TableHead>
                <TableHead className={cn(dataTableSortableHeadClass, 'text-right')} onClick={() => handleSort('paymentAmount')}>
                  <div className="flex items-center justify-end">Payment Amt. {getSortIcon('paymentAmount')}</div>
                </TableHead>
                <TableHead className={dataTableHeadClass}>Payment Mode</TableHead>
                <TableHead className={dataTableHeadClass}>Payment Date</TableHead>
                <TableHead className={dataTableHeadClass}>Reference No.</TableHead>
                <TableHead className={cn(dataTableSortableHeadClass, 'text-right')} onClick={() => handleSort('discountAmt')}>
                  <div className="flex items-center justify-end">Write-off Amt. {getSortIcon('discountAmt')}</div>
                </TableHead>
                <TableHead className={cn(dataTableSortableHeadClass, 'text-right')} onClick={() => handleSort('settledAmount')}>
                  <div className="flex items-center justify-end">Settlement Amt. {getSortIcon('settledAmount')}</div>
                </TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading settlements…" />
              ) : paginated.length > 0 ? (
                paginated.map((lead, idx) => (
                  <TableRow key={lead.id} className={dataTableBodyRowClass}>
                    ...
                    <TableCell className={dataTableCellClass}>{lead.paymentMode || '—'}</TableCell>
                    <TableCell className={cn(dataTableCellClass, 'uppercase')}>
                       {lead.paymentDate ? formatAppDateOrFallback(lead.paymentDate, '-') : '-'}
                    </TableCell>
                    <TableCell className={dataTableCellClass}>{lead.refNo || '—'}</TableCell>
                    <TableCell className={cn(dataTableCellClass, 'text-right')}>
                      {formatCurrency(lead.discountAmt || 0)}
                    </TableCell>
                    <TableCell className={cn(dataTableCellClass, 'text-right')}>
                      {formatCurrency(lead.settledAmount || 0)}
                    </TableCell>
                    <TableCell className={cn(dataTableCellClass, 'text-center')}>
                       <Badge className={leadPipelineStatusBadgeClass(lead.status)}>
                          {lead.status}
                       </Badge>
                    </TableCell>
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 text-center">
                    <p className="text-slate-400 font-bold text-sm">No settlement records found</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
