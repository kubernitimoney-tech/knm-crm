...
export const RejectedSanctionsPage = () => {
  useTitle('Rejected Sanctions');
  ...
  const { rows: rejectedRows, isLoading, refetch } = usePipelineRows('sanction-rejected');
  ...
  const tableColSpan = actionCol.showColumn ? 11 : 10;
  ...
  const stats = useMemo(() => {
    return {
      ...
      lowCibil: rejectedRows.filter(l => l.rejectionReason === 'Low CIBIL Score').length,
      documentation: rejectedRows.filter(l => l.rejectionReason === 'Documentation Incomplete').length
    };
  }, [rejectedRows]);

  // Filtering & Sorting Logic
  const filteredAndSorted = useMemo(() => {
    const result = rejectedRows.filter(lead => {
      const matchesSearch =
    ...
    return result;
  }, [rejectedRows, searchQuery, dateFilter, dateRange, sortConfig]);
  ...
  const handleExport = () => {
    ...
    exportRowsToExcel(
      headers,
      data,
      buildExportFilename('sanction_rejected', periodFilterSlug(dateFilter)),
    );
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Rejected Sanctions</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">Repository of declined loan applications</p>
        </div>
          ...
          <Table>
            <TableHeader className={dataTableHeaderClass}>
              <TableRow className={dataTableHeaderRowClass}>
                ...
                <TableHead className={dataTableHeadClass}>Rejected By</TableHead>
                ...
                <TableHead className={cn(dataTableSortableHeadClass, 'text-center')} onClick={() => handleSort('loanAmount')}>
                  <div className="flex items-center justify-center">Loan Amt. {getSortIcon('loanAmount')}</div>
                </TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading rejected sanctions…" />
              ) : paginated.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 p-0">
                    <div className="flex min-h-64 w-full flex-col items-center justify-center py-12 text-center">
                      ...
                      <p className="text-slate-400 font-bold text-sm">No rejected records found</p>
