...
export const SanctionsPage = () => {
  useTitle('Approved Sanctions');
  ...
  const { rows: sanctionRows, isLoading, refetch } = usePipelineRows('sanction-approved');
  ...
  const tableColSpan = actionCol.showColumn ? 17 : 16;
  ...
  const stats = useMemo(() => {
    return {
      totalAmount: sanctionRows.reduce((acc, curr) => acc + curr.loanAmount, 0),
      avgCibil: sanctionRows.length
        ? Math.round(sanctionRows.reduce((acc, curr) => acc + curr.cibil, 0) / sanctionRows.length)
        : 0,
      total: sanctionRows.length
    };
  }, [sanctionRows]);

  // Filtering & Sorting Logic
  const filteredAndSortedSanctions = useMemo(() => {
    const result = sanctionRows.filter(lead => {
      const matchesSearch =
    ...
    return result;
  }, [sanctionRows, searchQuery, dateFilter, dateRange, sortConfig]);

  // Pagination Logic
  const paginatedSanctions = filteredAndSortedSanctions.slice(
  ...
  const handleExport = () => {
    ...
    const data = filteredAndSortedSanctions.map((lead, index) => [
      index + 1,
      lead.leadId,
      lead.customerName,
      lead.assignedCM,
    ...
    exportRowsToExcel(
      headers,
      data,
      buildExportFilename('sanction_approved', periodFilterSlug(dateFilter)),
    );
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Sanction Approved</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">All loan applications with sanction approval recorded</p>
        </div>
      ...
      <div className="space-y-3">
        <div className={dataTableFilterToolbarClass}>
          <DataTableSearchInput
            placeholder={searchPlaceholder('Name', 'Mobile', 'PAN', 'or Lead ID')}
            value={searchQuery}
            onChange={(value) => {
          ...
          <Table>
            <TableHeader className={dataTableHeaderClass}>
              <TableRow className={dataTableHeaderRowClass}>
                ...
                <TableHead className={dataTableHeadClass}>Sanction By CM</TableHead>
                ...
                <TableHead className={dataTableSortableHeadClass} onClick={() => handleSort('loanAmount')}>
                  <div className="flex items-center">Loan Amt. {getSortIcon('loanAmount')}</div>
                </TableHead>
                ...
                <TableHead className={dataTableSortableHeadClass} onClick={() => handleSort('repayDate')}>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading sanctions…" />
              ) : paginatedSanctions.length > 0 ? (
                paginatedSanctions.map((lead, idx) => (
                  <TableRow key={lead.id} className={dataTableBodyRowClass}>
                    ...
                    <TableCell className={dataTableCellClass}>
                      <Badge className={badgeClass('success')}>
                        {lead.status}
                      </Badge>
                    </TableCell>
                    <TableCell className={cn(dataTableCellClass, 'text-right')}>
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 p-0">
                    <div className="flex min-h-64 w-full flex-col items-center justify-center py-12 text-center">
                      ...
                      <p className="text-slate-400 font-bold text-sm">No sanction data found</p>
        ...
        <DataTableFooter
          ...
          totalItems={filteredAndSortedSanctions.length}
