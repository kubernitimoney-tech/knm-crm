...
export const DisbursedLoansPage = () => {
  ...
  const stats = useMemo(() => {
    return {
      total: disbursedRows.length,
      amount: disbursedRows.reduce((acc, curr) => acc + (curr.disbursedAmount || 0), 0)
    };
  }, [disbursedRows]);

  // Filtering & Sorting Logic
  const filteredAndSorted = useMemo(() => {
    const result = disbursedRows.filter(lead => {
      const matchesSearch =
    ...
    return result;
  }, [disbursedRows, searchQuery, dateFilter, dateRange, sortConfig]);
  ...
  const handleExport = () => {
    ...
    exportRowsToExcel(headers, data, `disbursed_loans_export_${format(new Date(), 'yyyy-MM-dd')}`);
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <Breadcrumbs />
          <h1 className="text-[22px] font-black text-primary-deep leading-tight">Disbursed loans</h1>
          <p className="text-mid-shade text-xs mt-1 font-medium italic">Record of completely processed and disbursed loan accounts</p>
        </div>
      ...
      <div className="space-y-3">
        ...
        <div className="overflow-x-auto overflow-y-hidden">
          <Table>
            ...
            <TableBody>
              {isLoading ? (
                <TableLoadingRow colSpan={tableColSpan} message="Loading disbursed loans…" />
              ) : paginated.length > 0 ? (
                ...
                <TableRow>
                  <TableCell colSpan={tableColSpan} className="h-64 text-center">
                    <p className="text-slate-400 font-bold text-sm">No disbursed loan records found</p>
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
