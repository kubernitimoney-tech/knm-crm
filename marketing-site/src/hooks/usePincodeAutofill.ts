...
import { digitsOnly, fetchPincodeDetails, type PincodeDetails } from '@/lib/pincodeApi';
    ...
    (
      ...
      debounceRef.current = setTimeout(async () => {
        ...
        try {
          ...
          if (requestId === requestIdRef.current) {
            onError?.(
              err instanceof Error ? err.message : 'Could not find details for this PIN code.',
            );
          }
        } finally {
          if (requestId === requestIdRef.current) {
